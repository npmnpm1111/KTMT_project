...
.eqv DISPLAY_READY 0xFFFF0008  		# =1 if the display has already to do                                  
                                  	# Auto clear after sw 
.text    
    li $k0, KEY_CODE     		# TEST KEY     
    li $k1, KEY_READY    		  
    li $s2, DISPLAY_CODE    		# 
    li $s1, DISPLAY_READY    		# TEST DISPLAY
    
    addi    $s5, $0, 512            	#store the width in s5     	
    #pass the colour through to s0 - for every param the colour is s0, a0-3 are the xy + size params
    #circle:
    addi    $a0, $0, 256        	#x = 256
    addi    $a1, $0, 256        	#y = 256    
    addi    $a2, $0, 20         	#r = 20
    addi    $a3, $0, 2			#default move distance
    addi    $s0, $0, 0x00FFFF66
    jal DrawCircle  
    nop
Input:
    ReadKey: lw $t0, 0($k0) 		# $t0 = [$k0] = KEY CODE
    j moving
moving:
    beq $t0, 97, left
    beq $t0, 100, right
...
SetPixel:
	...
	mul  	$a2, $a2, $s5			#multiply by width (s7 declared at top of program, never saved and loaded and it should never be changed)
